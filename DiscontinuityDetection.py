...
def find_filtering_windows(troubled_cells, gap=4, kernel_size_by_cell=2, N=128):
    if len(troubled_cells) == 1:
        return [troubled_cells], [np.arange(max(1, troubled_cells[0]-kernel_size_by_cell+1) , min(troubled_cells[0]+kernel_size_by_cell+1, N)+1)] 
    else: 
        ...
        if ind_set == []:
            ...
            filtering_windows = [np.arange(max(1, troubled_cells[0]-kernel_size_by_cell + 1), min(troubled_cells[-1]+kernel_size_by_cell+1, N)+1)]
            ...
            for i in range(len(ind_set)): 
                ...
                disc_window = np.arange(max(1, troubled_cells[prev_ind]-kernel_size_by_cell+1), min(troubled_cells[next_ind-1]+kernel_size_by_cell+1, N)+1)
                ...
                if i == len(ind_set)-1:
                    ...
                    disc_window = np.arange(troubled_cells[prev_ind]-kernel_size_by_cell+1, min(troubled_cells[-1]+kernel_size_by_cell+1, N)+1)
                    filtering_windows.append(disc_window)
